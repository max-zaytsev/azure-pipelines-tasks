...
function commitChanges(directory, pathToAdd, branch, commitMessage) {
  ...
  execInForeground(
    `${GIT} -c credential.helper='!f() { echo "username=pat"; echo "password=$TOKEN"; };f' push --set-upstream origin ${branch}`,
    directory
  );
}
...
async function commitAzureDevOpsChanges(pathToAdoRepo, taskName) {
  ...
  const branch = `users/${username}/m${currentSprint}/${taskName}-UpdateUnifiedDeps-new5`;
  ...
  commitChanges(pathToAdoRepo, pathToUnifiedDependencies, branch, commitMessage);
}
